...
import { ActionButton, GlowingButton, ShinyButton } from "@repo/ui/components/buttons";
    ...
    <>
      {!hidden && (
        <div className=" w-1/2 p-5 bg-[#252a2c] relative">
          ...
          <button className="bg-[#0C271B] w-1/2 p-2 rounded-lg" onClick={() => onButtonClick('Buy')}>Buy</button>
          <button className="bg-[#181C1E] w-1/2 text-[#565D6D] p-2 rounded-lg" onClick={() => onButtonClick('Sell')}>Sell</button>
          </div>
          <div>
              <p>NFT to Buy</p>
              <GlowingButton size="sm">Buy at {Price}</GlowingButton>
          </div>
